package §_-k1c§
{
   import buttons.§_-j18§;
   import flash.events.MouseEvent;
   import flash.text.TextFormat;
   import flash.text.TextFormatAlign;
   
   public class §_-c2J§ extends Dialog
   {
      
      private static var _instance:§_-c2J§;
      
      public function §_-c2J§()
      {
         super(gls("Монеты за друга!"));
         var _loc1_:CoinsForFriendImage = new CoinsForFriendImage();
         _loc1_.x = -48;
         addChild(_loc1_);
         var _loc2_:TextFormat = new TextFormat(null,12,4858895,true);
         _loc2_.align = TextFormatAlign.CENTER;
         var _loc3_:§_-22V§ = new §_-22V§(gls("Твой друг принял приглашение в игру.\nТвоё вознаграждение"),10,5,_loc2_);
         addChild(_loc3_);
         var _loc4_:§_-j18§ = new §_-j18§(gls("Забрать"));
         _loc4_.x = 90;
         _loc4_.y = 150;
         _loc4_.addEventListener(MouseEvent.CLICK,hide);
         addChild(_loc4_);
         place();
         this.width = 300;
         this.height = 230;
      }
      
      public static function show() : void
      {
         if(!_instance)
         {
            _instance = new §_-c2J§();
         }
         _instance.show();
      }
   }
}

