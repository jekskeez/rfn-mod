package §_-t1F§
{
   import §_-68§.§_-EI§;
   import §_-I10§.§_-51T§;
   import §_-I10§.§_-63q§;
   import §_-I10§.§_-S9§;
   import §_-T2y§.§_-71o§;
   import §_-Z24§.§_-L1d§;
   import §_-d10§.§_-RA§;
   import §_-e1G§.§_-Hb§;
   import §_-k1c§.Dialog;
   import buttons.§_-j18§;
   import com.api.Services;
   import flash.display.DisplayObject;
   import flash.display.Sprite;
   import flash.events.MouseEvent;
   import flash.filters.BevelFilter;
   import flash.filters.DropShadowFilter;
   import flash.filters.GlowFilter;
   import flash.text.TextFormat;
   
   public class §_-B24§ extends Dialog
   {
      
      public static const §_-d2o§:Array = [new BevelFilter(1,58,16724736,1,16763904,1,2,2),new GlowFilter(16777215,1,4,4,8),new DropShadowFilter(2,45,0,1,2,2,0.25)];
      
      private static var §_-q1R§:Object = {};
      
      private var type:int = -1;
      
      public function §_-B24§(param1:int)
      {
         var _loc10_:Sprite = null;
         this.type = param1;
         var _loc2_:§_-63q§ = §_-S9§.§_-b28§(param1);
         super(_loc2_.name,true);
         var _loc3_:DisplayObject = new DialogBundleBack();
         addChild(_loc3_);
         var _loc4_:§_-22V§ = new §_-22V§(this.text,0,0,new TextFormat(null,14,7027231,true));
         _loc4_.x = int((_loc3_.width - _loc4_.textWidth) * 0.5);
         addChild(_loc4_);
         _loc3_ = this.§_-T1t§;
         _loc3_.scaleX = _loc3_.scaleY = 1.2;
         _loc3_.x = 210 - int(_loc3_.width * 0.5);
         _loc3_.y = 80;
         addChild(_loc3_);
         _loc4_ = new §_-22V§(this.§_-M2r§,0,275,new TextFormat(§_-22V§.§_-pJ§,15,16733461));
         _loc4_.x = 210 - int(_loc4_.textWidth * 0.5);
         _loc4_.filters = §_-d2o§;
         addChild(_loc4_);
         var _loc5_:§_-RA§ = new §_-RA§();
         _loc5_.y = 20;
         _loc5_.§_-t2j§ = _loc2_.§_-t2j§;
         addChild(_loc5_);
         var _loc6_:Vector.<§_-51T§> = _loc2_.§_-T2F§;
         var _loc7_:Sprite = new Sprite();
         addChild(_loc7_);
         var _loc8_:int = 0;
         while(_loc8_ < _loc6_.length)
         {
            _loc10_ = new Sprite();
            _loc10_.x = _loc8_ < 3 ? 5 + _loc8_ % 3 * 140 : 35 + _loc8_ % 3 * 185;
            _loc10_.y = 320 + int(_loc8_ / 3) * 60;
            _loc7_.addChild(_loc10_);
            _loc3_ = new _loc6_[_loc8_].imageClass();
            _loc10_.addChild(_loc3_);
            new §_-Hb§(_loc3_,§_-L1d§.§_-Z15§(_loc6_[_loc8_].imageClass));
            _loc4_ = new §_-22V§(_loc6_[_loc8_].description,_loc3_.width + 5,0,§_-L1d§.§_-CD§);
            _loc4_.wordWrap = true;
            _loc4_.width = 145;
            _loc4_.y = (_loc3_.height - _loc4_.textHeight) * 0.5 - 2;
            _loc10_.addChild(_loc4_);
            _loc8_++;
         }
         var _loc9_:§_-j18§ = new §_-j18§(gls("В банк"));
         _loc9_.x = 210 - int(_loc9_.width * 0.5);
         _loc9_.y = 440;
         _loc9_.addEventListener(MouseEvent.CLICK,this.§_-P2v§);
         addChild(_loc9_);
         place();
         switch(this.type)
         {
            case §_-S9§.§_-y1V§:
            case §_-S9§.§_-Q2A§:
               this.width -= 70;
               this.height += 40;
         }
         this.§_-O2e§.x = this.width - 35;
      }
      
      public static function show(param1:int) : void
      {
         var type:int = param1;
         if(type in §_-q1R§)
         {
            return;
         }
         §_-q1R§[type] = true;
         §_-71o§.addCallback(function():void
         {
            §_-EI§.load(function():void
            {
               new §_-B24§(type).show();
            },true);
         });
      }
      
      private function get §_-T1t§() : DisplayObject
      {
         switch(this.type)
         {
            case §_-S9§.§_-y1V§:
               return new ImageBundleNewbieRich();
            case §_-S9§.§_-Q2A§:
               return new ImageBundleNewbiePoor();
            default:
               return null;
         }
      }
      
      private function get text() : String
      {
         switch(this.type)
         {
            case §_-S9§.§_-y1V§:
               return gls("Незаменимая помощь на первых порах!");
            case §_-S9§.§_-Q2A§:
               return gls("Помощь юным белкам по невероятно выгодной цене!");
            default:
               return "";
         }
      }
      
      private function get §_-M2r§() : String
      {
         switch(this.type)
         {
            case §_-S9§.§_-y1V§:
            case §_-S9§.§_-Q2A§:
               return gls("Ограниченное предложение!");
            default:
               return "";
         }
      }
      
      private function §_-P2v§(param1:MouseEvent) : void
      {
         hide();
         Services.bank.open();
         §_-L1d§.§_-6U§(this.type);
      }
      
      override protected function setDefaultSize() : void
      {
         this.§_-R1Y§ = 5;
         this.§_-4n§ = 5;
         this.topOffset = 10;
         this.§_-JZ§ = 0;
      }
   }
}

