package §_-vK§
{
   ...
   public class §_-Z2§ extends §_-q4§
   {
      
      public static const §_-cS§:Number = 10;
      
      public static const §_-H2y§:Number = 5;
      
      private static const §_-c1F§:Number = 20;
      
      private var §_-32J§:Object = {};
      
      private var radius:Number;
      
      public function §_-Z2§(param1:Hero, param2:Array)
      {
         super(param1,param2);
         this.§_-i2D§ = §_-QC§.§_-N4§;
      ...
      private function §_-nw§() : void
      {
         ...
         for each(_loc1_ in this.hero.game.squirrels.players)
         {
            ...
            {
               if(_loc1_.id in this.§_-32J§)
               {
                  _loc1_.jumpVelocity -= this.§_-32J§[_loc1_.id];
                  if(_loc1_.id != this.hero.id)
                  {
                     _loc1_.heroView.§_-W2h§();
                     _loc1_.removeBuff(this.buff);
                  }
                  delete this.§_-32J§[_loc1_.id];
               }
            }
            else if(!(_loc1_.id in this.§_-32J§))
            {
               _loc3_ = (§_-cS§ + (this.§_-41W§ ? §_-H2y§ : 0)) / 100 * _loc1_.jumpVelocity;
               this.§_-32J§[_loc1_.id] = _loc3_;
