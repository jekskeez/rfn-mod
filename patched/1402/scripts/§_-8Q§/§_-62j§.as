package §_-8Q§
{
   import Box2D.Collision.Shapes.b2CircleShape;
   import Box2D.Collision.b2Manifold;
   import Box2D.Dynamics.Contacts.b2Contact;
   import Box2D.Dynamics.Joints.b2JointEdge;
   import Box2D.Dynamics.b2Body;
   import Box2D.Dynamics.b2BodyDef;
   import Box2D.Dynamics.b2ContactImpulse;
   import Box2D.Dynamics.b2FixtureDef;
   import Box2D.Dynamics.b2World;
   import §_-TK§.§_-aS§;
   import §_-TK§.§_-f1u§;
   import §_-j4§.§_-Pn§;
   import §_-l2u§.§_-62b§;
   import game.mainGame.entity.§_-6D§;
   import protocol.Connection;
   import protocol.§_-s2l§;
   import protocol.§_-u1O§;
   
   public class §_-62j§ extends GameBody implements §_-Pn§
   {
      
      private static const §_-A2L§:int = 64;
      
      private static const §_-53X§:uint = 32;
      
      private static const §_-C3c§:uint = 8;
      
      private static const §_-G2l§:b2CircleShape = new b2CircleShape(§_-A2L§ / Game.§_-12A§);
      
      private static const §_-ql§:b2FixtureDef = new b2FixtureDef(§_-G2l§,null,0.2,0,0.001,§_-53X§,§_-C3c§,0,true);
      
      private static const §_-41z§:b2BodyDef = new b2BodyDef(true,false,b2Body.b2_dynamicBody);
      
      private var view:§_-f1u§ = null;
      
      private var radius:§_-aS§ = null;
      
      public function §_-62j§()
      {
         super();
         this.view = new §_-f1u§(new MagnetImg());
         this.view.loop = true;
         this.view.stop();
         this.view.y = this.view.height * 0.5;
         §_-J2J§(this.view);
         this.radius = new §_-aS§(new PerkRadius());
         this.radius.scaleXY(§_-A2L§ * 2 / this.radius.height);
         §_-J2J§(this.radius);
         this.fixed = true;
         Connection.listen(this.§_-x2f§,[§_-s2l§.§_-1q§]);
      }
      
      override public function build(param1:b2World) : void
      {
         this.body = param1.CreateBody(§_-41z§);
         this.body.SetUserData(this);
         this.body.CreateFixture(§_-ql§).SetUserData(this);
         super.build(param1);
         this.view.play();
      }
      
      override public function dispose() : void
      {
         if(this.view)
         {
            this.view.removeFromParent();
         }
         this.view = null;
         Connection.forget(this.§_-x2f§,[§_-s2l§.§_-1q§]);
         super.dispose();
      }
      
      override public function set showDebug(param1:Boolean) : void
      {
         super.showDebug = param1;
         this.radius.visible = this.§_-m2u§;
      }
      
      public function beginContact(param1:b2Contact) : void
      {
         var _loc2_:Hero = null;
         if(param1.GetFixtureA().GetBody().GetUserData() is Hero)
         {
            _loc2_ = param1.GetFixtureA().GetBody().GetUserData();
         }
         if(param1.GetFixtureB().GetBody().GetUserData() is Hero)
         {
            _loc2_ = param1.GetFixtureB().GetBody().GetUserData();
         }
         if(!_loc2_ || _loc2_.isDead || _loc2_.inHollow)
         {
            return;
         }
         var _loc3_:Array = [];
         var _loc4_:b2JointEdge = _loc2_.body.GetJointList();
         while(_loc4_)
         {
            if(_loc4_.joint.GetBodyA().GetUserData() is §_-6D§)
            {
               _loc3_.push([_loc2_.id,(_loc4_.joint.GetBodyA().GetUserData() as GameBody).id]);
            }
            else if(_loc4_.joint.GetBodyB().GetUserData() is §_-6D§)
            {
               _loc3_.push([_loc2_.id,(_loc4_.joint.GetBodyB().GetUserData() as GameBody).id]);
            }
            _loc4_ = _loc4_.next;
         }
         var _loc5_:* = int(_loc3_.length - 1);
         while(_loc5_ >= 0)
         {
            this.§_-V5§(_loc3_[_loc5_][0],_loc3_[_loc5_][1]);
            _loc5_--;
         }
      }
      
      public function endContact(param1:b2Contact) : void
      {
      }
      
      public function preSolve(param1:b2Contact, param2:b2Manifold) : void
      {
      }
      
      public function postSolve(param1:b2Contact, param2:b2ContactImpulse) : void
      {
      }
      
      private function §_-V5§(param1:int, param2:int) : void
      {
         if(param1 > 0 && param1 != Game.selfId)
         {
            return;
         }
         if(this.§_-21H§ is §_-62b§)
         {
            this.magnetize(param2);
         }
         else
         {
            Connection.§_-e2T§(§_-u1O§.§_-1q§,JSON.stringify({"magnetize":[this.id,param2]}));
         }
      }
      
      private function magnetize(param1:int) : void
      {
         if(this.§_-21H§ == null)
         {
            return;
         }
         var _loc2_:§_-6D§ = this.§_-21H§.map.getObject(param1) as §_-6D§;
         if(!_loc2_)
         {
            return;
         }
         _loc2_.magnetize(this);
      }
      
      private function §_-x2f§(param1:§_-s2l§) : void
      {
         var _loc2_:Object = param1[1];
         if("magnetize" in _loc2_)
         {
            if(_loc2_["magnetize"][0] != this.id)
            {
               return;
            }
            if(_loc2_["magnetize"][1])
            {
               this.magnetize(_loc2_["magnetize"][1]);
            }
         }
      }
   }
}

