package §_-j2E§
{
   ...
   public class §_-n1r§ extends §_-92f§
   {
      ...
      public function §_-n1r§(param1:Hero)
      {
         super(param1);
         this.§_-S2A§ = "push";
         this.§_-i1J§ = true;
      ...
      private function move(param1:Number) : void
      {
         ...
         for each(_loc2_ in this.elements)
         {
            if(_loc2_.body != null)
            {
               ...
               _loc3_.x = Math.max(0,Math.min(_loc3_.x,this.hero.game.map.size.x / Game.§_-12A§));
               _loc3_.y = Math.max(0,Math.min(_loc3_.y,this.hero.game.map.size.y / Game.§_-12A§));
