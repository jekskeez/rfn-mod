package flash
{
   import flash.display.MovieClip;
   import flash.display.Stage;
   import flash.events.Event;
   import flash.filters.GlowFilter;
   import flash.text.TextField;
   import flash.text.TextFieldAutoSize;
   import flash.text.TextFormat;
   import flash.utils.getQualifiedClassName;
   import flash.utils.setTimeout;
   
   public dynamic class Boot extends MovieClip
   {
      
      public static var tf:TextField;
      
      public static var §_-SG§:Array;
      
      public static var §_-B2g§:Error;
      
      public static var §_-PC§:Boolean = false;
      
      public function Boot()
      {
         if(Boot.§_-PC§)
         {
            return;
         }
         super();
      }
      
      public static function §_-11G§(param1:Object) : String
      {
         var _loc5_:* = null;
         if(param1.params == null)
         {
            return param1.tag;
         }
         var _loc2_:Array = [];
         var _loc3_:int = 0;
         var _loc4_:Array = param1.params;
         while(_loc3_ < int(_loc4_.length))
         {
            _loc5_ = _loc4_[_loc3_];
            _loc3_++;
            _loc2_.push(Boot.__string_rec(_loc5_,""));
         }
         return param1.tag + "(" + _loc2_.join(",") + ")";
      }
      
      public static function §_-5d§(param1:*, param2:*) : Boolean
      {
         var _loc4_:* = null;
         try
         {
            if(param2 == Dynamic)
            {
               return true;
            }
            return param1 is param2;
         }
         catch(_loc_e_:*)
         {
         }
      }
      
      public static function __clear_trace() : void
      {
         if(Boot.tf == null)
         {
            return;
         }
         Boot.tf.parent.removeChild(Boot.tf);
         Boot.tf = null;
         Boot.§_-SG§ = null;
      }
      
      public static function __set_trace_color(param1:uint) : void
      {
         var _loc2_:TextField = Boot.§_-S3§();
         _loc2_.textColor = param1;
         _loc2_.filters = [];
      }
      
      public static function §_-S3§() : TextField
      {
         var _loc2_:* = 0;
         var _loc3_:int = 0;
         var _loc4_:* = null as TextFormat;
         var _loc1_:MovieClip = §_-x1s§.§_-n1s§;
         if(Boot.tf == null)
         {
            Boot.tf = new TextField();
            _loc2_ = 16777215;
            _loc3_ = 0;
            if(_loc1_.stage != null)
            {
               _loc3_ = int(_loc1_.stage.color);
               _loc2_ = 16777215 - _loc3_;
            }
            Boot.tf.textColor = _loc2_;
            Boot.tf.filters = [new GlowFilter(_loc3_,1,2,2,20)];
            _loc4_ = Boot.tf.getTextFormat();
            _loc4_.font = "_sans";
            Boot.tf.defaultTextFormat = _loc4_;
            Boot.tf.selectable = false;
            Boot.tf.width = _loc1_.stage == null ? 800 : _loc1_.stage.stageWidth;
            Boot.tf.autoSize = TextFieldAutoSize.LEFT;
            Boot.tf.mouseEnabled = false;
         }
         if(_loc1_.stage == null)
         {
            _loc1_.addChild(Boot.tf);
         }
         else
         {
            _loc1_.stage.addChild(Boot.tf);
         }
         return Boot.tf;
      }
      
      public static function §_-525§(param1:*, param2:Object) : void
      {
         var _loc3_:TextField = Boot.§_-S3§();
         var _loc4_:String = param2 == null ? "(null)" : param2.fileName + ":" + int(param2.lineNumber);
         if(Boot.§_-SG§ == null)
         {
            Boot.§_-SG§ = [];
         }
         Boot.§_-SG§ = Boot.§_-SG§.concat((_loc4_ + ": " + Boot.__string_rec(param1,"")).split("\n"));
         _loc3_.text = Boot.§_-SG§.join("\n");
         var _loc5_:Stage = §_-x1s§.§_-n1s§.stage;
         if(_loc5_ == null)
         {
            return;
         }
         while(int(Boot.§_-SG§.length) > 1 && _loc3_.height > _loc5_.stageHeight)
         {
            Boot.§_-SG§.shift();
            _loc3_.text = Boot.§_-SG§.join("\n");
         }
      }
      
      public static function __string_rec(param1:*, param2:String) : String
      {
         var _loc5_:* = null as String;
         var _loc6_:* = null as Array;
         var _loc7_:* = null as Array;
         var _loc8_:* = 0;
         var _loc9_:* = null;
         var _loc10_:* = null as String;
         var _loc11_:Boolean = false;
         var _loc12_:int = 0;
         var _loc13_:int = 0;
         var _loc14_:* = null as String;
         var _loc4_:String = getQualifiedClassName(param1);
         _loc5_ = _loc4_;
         if(_loc5_ == "Object")
         {
            _loc8_ = 0;
            _loc7_ = [];
            _loc9_ = param1;
            for(_loc8_ in _loc9_)
            {
               _loc7_.push(_loc8_);
            }
            _loc6_ = _loc7_;
            _loc10_ = "{";
            _loc11_ = true;
            _loc8_ = 0;
            _loc12_ = int(_loc6_.length);
            while(_loc8_ < _loc12_)
            {
               _loc13_ = _loc8_++;
               _loc14_ = _loc6_[_loc13_];
               if(_loc14_ == "toString")
               {
                  try
                  {
                     return param1.toString();
                  }
                  catch(_loc_e_:*)
                  {
                  }
               }
               if(_loc11_)
               {
                  _loc11_ = false;
               }
               else
               {
                  _loc10_ += ",";
               }
               _loc10_ += " " + _loc14_ + " : " + Boot.__string_rec(param1[_loc14_],param2);
            }
            if(!_loc11_)
            {
               _loc10_ += " ";
            }
            return _loc10_ + "}";
         }
         if(_loc5_ == "Array")
         {
            if(param1 == Array)
            {
               return "#Array";
            }
            _loc10_ = "[";
            _loc11_ = true;
            _loc6_ = param1;
            _loc8_ = 0;
            _loc12_ = int(_loc6_.length);
            while(_loc8_ < _loc12_)
            {
               _loc13_ = _loc8_++;
               if(_loc11_)
               {
                  _loc11_ = false;
               }
               else
               {
                  _loc10_ += ",";
               }
               _loc10_ += Boot.__string_rec(_loc6_[_loc13_],param2);
            }
            return _loc10_ + "]";
         }
         _loc5_ = typeof param1;
         if(_loc5_ == "function")
         {
            return "<function>";
         }
         return new String(param1);
      }
      
      public static function §_-h4§(param1:String) : String
      {
         return param1;
      }
      
      public function start() : void
      {
         var _loc3_:* = null;
         var _loc2_:MovieClip = §_-x1s§.§_-n1s§;
         try
         {
            if(_loc2_ == this && _loc2_.stage != null && _loc2_.stage.align == "")
            {
               _loc2_.stage.align = "TOP_LEFT";
            }
         }
         catch(_loc_e_:*)
         {
            if(_loc2_.stage == null)
            {
               _loc2_.addEventListener(Event.ADDED_TO_STAGE,§_-b17§);
            }
            else if(_loc2_.stage.stageWidth == 0)
            {
               setTimeout(start,1);
            }
            else
            {
               init();
            }
            return;
         }
      }
      
      public function init() : void
      {
         Boot.§_-B2g§ = new Error();
         throw "assert";
      }
      
      public function §_-b17§(param1:*) : void
      {
         §_-x1s§.§_-n1s§.removeEventListener(Event.ADDED_TO_STAGE,§_-b17§);
         start();
      }
   }
}

Array.prototype;
_loc1_.setPropertyIsEnumerable("copy",false);
_loc1_.setPropertyIsEnumerable("insert",false);
_loc1_.setPropertyIsEnumerable("remove",false);
_loc1_.setPropertyIsEnumerable("iterator",false);

